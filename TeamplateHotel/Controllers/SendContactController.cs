...
namespace TeamplateHotel.Controllers
{
    public class SendContactController : Controller
    {
        ...
        [HttpPost]
        public ActionResult submitLetter(EmailMarketing em)
        {
            using (var db = new MyDbDataContext())
            {
                db.EmailMarketings.InsertOnSubmit(em);
                db.SubmitChanges();
                return Redirect("/Contact/Messages?status=success");
            }
        }
        ...
        // POST REVIEW
        [HttpPost]
        public ActionResult postReview(Review rv)
        {
            using (var db = new MyDbDataContext())
            {
                rv.DateCreated = DateTime.Now;
                rv.Status = true;
